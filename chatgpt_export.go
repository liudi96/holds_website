...
func buildChatGPTContextZip(state AppState, generatedAt time.Time) ([]byte, error) {
	...
	files := []struct {
		...
		{"institutional_research_protocol.md", renderInstitutionalResearchProtocol(meta)},
		{"research_quality_checklist.md", renderResearchQualityChecklist(meta)},
		{"industry_research_templates.md", renderIndustryResearchTemplates(meta)},
...
- 机构级个股研究必须先读取 ` + "`institutional_research_protocol.md`" + `、` + "`research_quality_checklist.md`" + ` 和 ` + "`industry_research_templates.md`" + `；先完成完整研究报告，再输出可导入 JSON。
...
- 单股研究：先给出完整研究报告，覆盖利润驱动因子地图、敏感性测算、三情景估值、操作纪律和检查清单结论。
...
- 导入 JSON 前必须先完成 institutional_research_protocol.md 和 research_quality_checklist.md；JSON 不承载完整研究报告。
...
func renderInstitutionalResearchProtocol(meta string) string {
	return meta + `# 机构级价值投资研究流程

本文件定义 ChatGPT 做个股深度研究时必须遵循的工作流。完整研究报告由 ChatGPT 对话承接；网站只导入最后的摘要 JSON。

## 一、研究顺序

1. 先读取本 ZIP 的组合档案：项目说明、参考表、组合快照、双策略表、风险委员会备忘录。
2. 再读取目标股票的 stocks/{symbol}.md，提取成本、仓位、历史结论、目标买入价、内在价值、安全边际、执行纪律、风险触发项、最近研究更新和财报 PDF。
3. 然后核验最新外部信息：股价、市值、最新财报、分红、回购、重大新闻、行业数据、监管与政策变化。
4. 按行业模板建立利润驱动因子地图，不允许只复述财报。
5. 完成收入端、成本端、政策端、竞争端、资本配置端拆解。
6. 对关键变量做敏感性测算，并说明哪些变量足以改变内在价值或操作纪律。
7. 给出悲观、中性、乐观三种内在价值区间，并解释假设差异。
8. 计算当前价格下安全边际，结合组合仓位和既有纪律给出买入、持有、加仓、减仓或观察动作。
9. 最后输出网站可导入 JSON；JSON 只保存核心结论，不承载完整研究报告。

## 二、研究报告必须包含

- 当前结论：一句话说明是否值得新资金买入、继续持有、等待、减仓或仅观察。
- 旧档案复核：旧内在价值、目标买入价、安全边际、历史动作、仓位和风险触发项是否仍成立。
- 最新事实：股价、市值、财报、分红、回购、新闻、政策和行业数据的日期与来源。
- 利润驱动因子地图：收入、成本、政策、竞争、资本配置分别列出关键变量。
- 敏感性测算：至少覆盖 3 个最可能改变内在价值的变量。
- 三情景估值：悲观、中性、乐观价值区间、概率或权重、核心假设。
- 普通股东现金流：自由现金流、少数股东分流、维持性资本开支和分红/回购覆盖。
- 反证条件：什么事实出现时必须下修估值、暂停买入或退出观察。
- 导入 JSON：根据研究性质选择 fullReview 或 eventUpdate。

## 三、输出纪律

- 不得跳过旧档案读取；如果旧档案缺失，要明确说明缺失项。
- 不得把短期价格变化当作结论，除非它改变安全边际或风险收益比。
...
func renderResearchQualityChecklist(meta string) string {
	return meta + `# 研究质量检查清单

ChatGPT 输出导入 JSON 前，必须逐项完成本检查清单。未覆盖的项目要在研究报告中说明原因，不能假装已核验。

## 1. 旧档案读取

- 已读取目标股票的 stocks/{symbol}.md。
- 已记录当前成本、股数、仓位、市值、浮动盈亏。
- 已复核旧内在价值、公允区间、目标买入价、观察价、首买价、重仓价。
- 已复核旧安全边际、质量分、长期股东评分、净现金和普通股东 FCF。
- 已复核旧 action、status、risk、plan、killCriteria、recent researchUpdates。

## 2. 最新事实核验

- 最新股价、市值、交易货币和日期已核验。
- 最新年报、半年报或季报已核验，并说明报告期。
- 最新分红、回购、派息率、综合回报率已核验。
- 最新重大新闻、监管、政策、行业数据已核验。
- 若数据冲突，已说明采用哪个来源和原因。

## 3. 利润驱动因子地图

- 收入端：销量、价格、产品结构、区域、渠道、客户、订阅/合约或项目节奏。
- 成本端：原材料、人工、能源、物流、折旧、维持性资本开支、促销和费用率。
- 政策端：税费、监管、补贴、牌照、环保、贸易、价格管制和反垄断。
- 竞争端：份额、替代品、价格战、客户议价、供应商议价、渠道变化。
- 资本配置端：分红、回购、并购、扩张、净现金、债务、少数股东分流。

## 4. 敏感性测算

- 至少列出 3 个关键变量的悲观/中性/乐观假设。
- 说明变量变化对收入、毛利率、经营利润、FCF 或估值倍数的影响路径。
- 明确哪些变量只是短期扰动，哪些变量会改变长期内在价值。

## 5. 三情景估值

- 悲观情景：收入、利润率、现金流、估值倍数或 DCF 假设下修。
- 中性情景：最可能经营假设，作为 intrinsicValue 的主要来源。
- 乐观情景：上行空间来源，不能只依赖估值倍数扩张。
- 当前价格下安全边际已按最新价格重新计算。

## 6. 操作纪律

- 给出买入、持有、加仓、减仓或观察结论。
- 明确触发价位、仓位上限、等待条件和复盘条件。
- 若结论改变，说明相对旧档案改变了什么。
- 若结论不变，说明哪些新事实不足以改变纪律。

## 7. JSON 输出前声明

在导入 JSON 前，研究报告末尾必须写明：
...
func renderIndustryResearchTemplates(meta string) string {
	return meta + `# 行业研究模板

本文件用于补足通用财务分析容易遗漏的行业变量。ChatGPT 研究目标股票时，先按行业选择最接近的模板，再结合公司实际业务调整。

## 饮料装瓶 / 中国食品

- 收入端：销量、ASP、产品结构、可口可乐授权区域、即饮渠道、餐饮渠道、现代渠道、电商、区域扩张。
- 成本端：浓缩液采购、糖/甜味剂、铝罐、PET、纸箱、物流、人工、能源、促销费用。
- 政策端：糖税、控糖政策、健康标签、包装回收、塑料限制、环保要求、食品安全监管。
- 竞争端：碳酸饮料成熟度、无糖茶/功能饮料替代、区域装瓶商竞争、渠道议价。
- 资本配置：少数股东权益分流、营运资本、产线投资、分红覆盖、普通股东 FCF。
- 敏感性重点：糖价、铝价、PET、毛利率、销量增速、费用率、少数股东分流。

## 乳制品

- 收入端：液态奶、奶粉、奶酪、冷饮、渠道库存、价格带和产品结构。
- 成本端：原奶价格、包材、冷链物流、促销费用、渠道费用。
- 政策端：食品安全、生育率、婴配粉注册、牧场环保和补贴。
- 竞争端：蒙牛/伊利/区域品牌、低温奶替代、渠道价格战。
- 敏感性重点：原奶价格、液奶销量、毛利率、销售费用率、奶粉恢复速度。

## 白酒

- 收入端：出厂价、批价、销量、产品结构、直营/经销、渠道库存。
- 成本端：基酒、包材、销售费用、渠道返利。
- 政策端：公务消费、税制、反腐、消费税和地方政策。
- 竞争端：高端/次高端价格带、批价稳定性、年轻消费替代。
- 敏感性重点：批价、销量、渠道库存、净利率、估值倍数。

## 家电 / 智能制造

- 收入端：内销、外销、品类结构、楼宇科技、机器人、ToB 项目。
- 成本端：铜、铝、钢、塑料、芯片、汇率、关税、物流。
- 政策端：家电补贴、出口贸易、关税、能效标准。
- 竞争端：价格战、渠道份额、海外品牌、ODM/OEM 变化。
- 敏感性重点：原材料、汇率、海外关税、毛利率、费用率、回购和分红。

## 银行

- 收入端：净息差、贷款规模、手续费、财富管理、信用卡。
- 成本端：存款成本、拨备、信用成本、资本消耗。
- 政策端：利率、让利、房地产、地方债、资本充足率监管。
- 竞争端：零售获客、存款竞争、财富管理替代。
- 敏感性重点：净息差、不良率、拨备覆盖率、ROE、PB 估值。

## 物业管理

- 收入端：基础物管、非业主增值、社区增值、在管面积、合约面积。
- 成本端：人工、外包、应收账款、合同资产、减值。
- 政策端：物业费提价、地产链信用、地方监管。
- 竞争端：央企/民企物管、地产母公司支持、第三方拓展。
- 敏感性重点：毛利率、收缴率、应收账款、减值、经营现金流。

## 能源 / 上游油气

- 收入端：油价、气价、产量、储量、成本桶、项目投产。
- 成本端：资本开支、折旧、税费、勘探成本、汇率。
- 政策端：资源税、能源安全、环保和地缘政治。
- 竞争端：低成本资源、储量接续、替代能源。
- 敏感性重点：Brent 油价、产量、资本开支、股息政策、周期估值倍数。

## 互联网平台

- 收入端：广告、游戏、金融科技、云、会员、生态交易额。
- 成本端：流量、内容、AI 算力、研发、云基础设施。
- 政策端：平台监管、游戏版号、数据安全、反垄断、地缘限制。
- 竞争端：短视频、AI、内容生态、支付和广告预算迁移。
- 敏感性重点：广告增速、游戏流水、AI capex、回购、FCF 转化率。

## 中医药 / 品牌药

- 收入端：核心品种、渠道、区域、零售/批发、产品提价能力。
- 成本端：中药材、人工、渠道费用、存货减值。
- 政策端：医保控费、集采、中药保护、香港/内地监管。
- 竞争端：品牌老化、替代药品、渠道议价。
- 敏感性重点：毛利率、核心品种销量、渠道恢复、现金和金融资产质量。

## 新能源汽车

- 收入端：交付量、ASP、车型结构、单车毛利、出口。
- 成本端：电池、智能驾驶、研发、销售网络、补贴。
- 政策端：新能源补贴、牌照、自动驾驶监管、贸易壁垒。
- 竞争端：价格战、品牌定位、车型周期、技术路线。
- 敏感性重点：销量、毛利率、扣非利润、经营现金流、政府补助依赖。

## 殡葬服务

- 收入端：墓园销售、殡仪服务、ASP、区域供给。
- 成本端：土地、维护、销售费用、商誉/资产减值。
- 政策端：民政监管、价格管制、公益属性、土地政策。
- 竞争端：区域牌照、服务替代、公益墓地供给。
- 敏感性重点：ASP、销售面积、审计意见、治理风险、现金真实性。
`
}
...
2. 再读 ` + "`institutional_research_protocol.md`" + `、` + "`research_quality_checklist.md`" + ` 和 ` + "`industry_research_templates.md`" + `，明确研究流程、质量门槛和行业变量。
3. 再读目标股票的 ` + "`stocks/{symbol}.md`" + `，确认已有结论、估值、执行纪律、最近研究更新和财报材料。
4. 如果要回写网站，最后读 ` + "`import_schema.md`" + `，只输出一个 JSON 对象。
...
- 先输出完整研究报告，再输出导入 JSON；不要用 JSON 代替研究报告。
- 输出 JSON 前必须声明研究质量检查清单已覆盖，或者列明缺口。
