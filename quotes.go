...
func fetchYahooMonthlyCloses(client *http.Client, symbol string) ([]dailyClose, error) {
	sourceSymbol := yahooSymbol(symbol)
	endpoint := "https://query1.finance.yahoo.com/v8/finance/chart/" + url.PathEscape(sourceSymbol) + "?range=10y&interval=1mo"

	req, err := http.NewRequest(http.MethodGet, endpoint, nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("User-Agent", "holds-website valuation history updater")
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 512))
		return nil, fmt.Errorf("valuation history request failed: %s %s", resp.Status, strings.TrimSpace(string(body)))
	}
	...
	if payload.Chart.Error != nil || len(payload.Chart.Result) == 0 {
		return nil, errors.New("empty valuation history response")
	}

	result := payload.Chart.Result[0]
	if len(result.Indicators.Quote) == 0 {
		return nil, errors.New("missing valuation history close series")
	}
	...
	if len(validCloses) == 0 {
		return nil, errors.New("no valid monthly close prices")
	}
