...
目标：云服务器负责定时调用后端完整更新接口，更新股票行情、基金净值、ETF 追踪和从 2026-07-07 开始的日盈亏记录。总览页仍会按冷却窗口后台刷新股票行情，但完整更新节奏以 systemd timer 为准。
...
- 页面进入总览页只会按冷却窗口刷新股票行情；基金净值、股息数据、ETF 追踪和日盈亏补齐由定时任务的完整更新接口负责。
